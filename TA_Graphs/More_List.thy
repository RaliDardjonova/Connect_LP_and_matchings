...
next
  ...
  from nths_eq_ConsD[of xs I a "ys @ bs"] Cons.prems obtain ys' zs' where
    ...
    by auto
  moreover from Cons.IH[OF \<open>nths zs' _ = _\<close>] 
  obtain ys'' zs'' where " zs' = ys'' @ zs'' \<and>
     nths ys'' {i - length ys' - 1 |i. i \<in> I \<and> length ys' < i} = ys \<and>
     nths zs''
      {i - length ys'' |i.
  ...
  ultimately show ?case
    apply (inst_existentials "ys' @ a # ys''" zs'')
      apply (simp; fail)
    subgoal
      apply (simp add: nths_out_of_bounds nths_append nths_Cons)
      using arg_cong2[of ys'' ys'' _ _ nths]  
    proof -
      have "{i - Suc (length ys') |i. i \<in> I \<and> length ys' < i} = {j. Suc (j + length ys') \<in> I}"
        using Suc_le_eq by fastforce
      show " xs = ys' @ a # ys'' @ zs'' \<Longrightarrow>
    length ys' \<in> I \<Longrightarrow>
    \<forall>i\<in>I. length ys' \<le> i \<Longrightarrow>
    nths zs''
     {j. \<exists>i. j + length ys'' = i - Suc (length ys') \<and>
    ...
    nths zs''
     {i - length ys'' |i.
      (\<exists>ia. i = ia - Suc (length ys') \<and>
            ia \<in> I \<and> length ys' < ia) \<and>
      length ys'' \<le> i} =
    bs \<Longrightarrow>
    (\<And>c d. ys'' = ys'' \<Longrightarrow>
            c = d \<Longrightarrow> nths ys'' c = nths ys'' d) \<Longrightarrow>
    nths ys'' {j. Suc (j + length ys') \<in> I} = ys" 
        using \<open>{i - Suc (length ys') |i. i \<in> I \<and> length ys' < i} = {j. Suc (j + length ys') \<in> I}\<close> by force
    qed
    subgoal
