...
proof -
  ...
  proof
    fix i
    show " i < dim_row A \<longrightarrow> (\<forall>j<dim_col A. A $$ (i, j) \<in> {- 1, 0, 1})"
    proof
      assume "i < dim_row A"
      show "(\<forall>j<dim_col A. A $$ (i, j) \<in> {- 1, 0, 1})"
      proof 
        fix j
        show "j < dim_col A \<longrightarrow> A $$ (i, j) \<in> {- 1, 0, 1}"
        proof
          assume "j < dim_col A"
          have "det (submatrix A {i} {j}) \<in> {- 1, 0, 1}" 
            using assms tot_unimodular_def by blast
          have "{k. k<dim_row A \<and> k\<in>{i}} = {i}" 
            using \<open>i < dim_row A\<close> by blast
          have "{k. k<dim_col A \<and> k\<in>{j}} = {j}" 
            using \<open>j < dim_col A\<close> by blast
          have "(submatrix A {i} {j}) = mat 1 1 ((\<lambda>(i1,j1). A $$ (pick {i} i1, pick {j} j1)))"
            unfolding submatrix_def
            using \<open>{k. k < dim_col A \<and> k \<in> {j}} = {j}\<close> \<open>{k. k < dim_row A \<and> k \<in> {i}} = {i}\<close> by force
          then have "det (submatrix A {i} {j}) = (submatrix A {i} {j}) $$ (0,0)"

            using determinant_one_element 
            by (metis mat_carrier)
          have "(submatrix A {i} {j}) $$ (0,0) =  A $$ (pick {i} 0, pick {j} 0)" 
            by (simp add: \<open>submatrix A {i} {j} = Matrix.mat 1 1 (\<lambda>(i1, j1). A $$ (pick {i} i1, pick {j} j1))\<close>)
          have "pick {i} 0 = i" 
            by (metis card_eq_1_iff less_one pick_in_set singletonD)
          have "pick {j} 0 = j" 
            by (metis card_eq_1_iff less_one pick_in_set singletonD)
          then have "det (submatrix A {i} {j}) = A $$ (i, j)" 
            using \<open>Determinant.det (submatrix A {i} {j}) = submatrix A {i} {j} $$ (0, 0)\<close> \<open>pick {i} 0 = i\<close> \<open>submatrix A {i} {j} $$ (0, 0) = A $$ (pick {i} 0, pick {j} 0)\<close> by presburger
          then show "A $$ (i, j) \<in> {- 1, 0, 1}" 
            using \<open>Determinant.det (submatrix A {i} {j}) \<in> {- 1, 0, 1}\<close> by presburger
        qed
      qed
    qed
  qed
...
proof -
  ...
  have 1:"1\<^sub>v (card (Vs E)) \<in> \<int>\<^sub>v" 
    by (simp add: gram_schmidt.fkjpkhp)
  have 2:"(insidence_mat E) \<in> carrier_mat (card (Vs E)) (card E)" 
    using dim_col_insidence_mat dim_row_insidence_mat by blast
  have 3:"insidence_mat E \<in> \<int>\<^sub>m" using totally_unimod_mat_int 
    using \<open>tot_unimodular (insidence_mat E)\<close> by blast
  then show "gram_schmidt_floor.int_polyh (card E) 
            (gram_schmidt_floor.pos_polyhedron (card E) (insidence_mat E) (1\<^sub>v (card (Vs E))))"
    using gram_schmidt_floor.tot_unimod_then_int_polyhedron[OF 2 3 4] 
